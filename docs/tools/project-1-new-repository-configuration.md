...
For a repository with one maintainer who is also its only CODEOWNER, use two layered rulesets. Put the approval and CODEOWNERS requirements in a review-protection ruleset and configure that exact GitHub user ID as its `User` bypass actor in `pull_request` mode. Put required checks, linear history, force-push denial, and deletion denial in a second ruleset with no bypass actors. This is needed because GitHub does not allow an author to approve their own pull request, while a bypass applies to every rule in the ruleset that grants it. The split lets the maintainer bypass only the impossible review while GitHub still enforces CI and history controls. Do not use an `always` bypass, a wildcard actor, or a broad role. Remove the review exception when an independent eligible reviewer becomes available.
