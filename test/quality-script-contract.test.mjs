...
const forbidden = /--fix\b|--write\b|--passWithNoTests|--forceExit|\|\|\s*true\b|\bexit\s+0\b|Error:\s*no test specified/;
...
for (const [repository, metadata] of Object.entries(contract.repositories)) {
  test(`${repository} exposes the truthful quality script interface`, async () => {
    ...
    for (const name of contract.requiredScripts) assert.equal(typeof scripts[name], "string", `${repository}: missing ${name}`);
    assertCheckGraph(repository, scripts);
    for (const name of contract.qualityOrder) assert.match(scripts["quality:check"], new RegExp(`(?:pnpm|npm|yarn)(?: run)? ${name.replace(":", "\\:")}`));
...
test("the reusable workflow invokes every check-only command", async () => {
  ...
  for (const name of contract.qualityOrder) assert.match(workflow, new RegExp(`run: \\\$\\{\\{ inputs\\.package-manager \\\}\\} run ${name.replace(":", "\\:")}`));
